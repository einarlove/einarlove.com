function Game(){
	...
	this.start = function(){
		...
		var light = new THREE.AmbientLight( 0x999999 ); // white light
		scene.add( light );

		var directionalLight = new THREE.DirectionalLight( 0xffff00, 0.2 );
		directionalLight.position.set(0, globe.size + 5000, 0);
	...
	function calculateVelocity(rotation){
		...
		if(player.rotation.x < 0)
			player.velocity.y = player.rotation.x * 8;
		else
			player.velocity.y = player.rotation.x * 2;
	...
	function updatePlayerPosition(){
		...
		// if((player.position.y < globe.size + 100)&&(player.rotation.x < 0)){
			// player.rotation.x += 0.1;
			// console.log(player.rotation.x);
		// }

		if(player.position.y < globe.size + 10){
			player.position.y = globe.size + 10;
			player.velocity.y = 0;
		}
	...
	function updateCameraPosition(){
		...
		var ac = game.quadratic(0, 0, 40, Math.abs(player.rotation.x/2));
	...
	function updateAnimations(){
		...
		if(player.requestGlide){
			...
			player.animation.duration = 1667 + (player.velocity.y * 150);
